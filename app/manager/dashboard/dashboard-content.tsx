...
export async function ManagerDashboardContent() {
  ...
  const { data: recentOrders } = await supabase
    .from("orders")
    .select("id, status, total_cents, created_at, guest_name, customer_id")
        ...
        <section className="overflow-hidden rounded-lg border border-[hsl(35,20%,90%)] bg-white shadow-sm">
          ...
          <div className="p-6">
            ...
            {staffDetails && staffDetails.length > 0 ? (
              ...
              <p className="text-sm text-[hsl(25,35%,45%)]">No staff members</p>
            )}
          </div>
        </section>
      </div>
                ...
                {mostSoldItems.map((item, index) => (
                  ...
                  >
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-xs text-[hsl(25,35%,45%)]">Revenue</p>
                    </div>
                  </div>
                ))}
              </div>
