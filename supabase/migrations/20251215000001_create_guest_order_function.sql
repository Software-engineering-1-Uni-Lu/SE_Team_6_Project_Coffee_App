-- Create a Postgres function to handle guest orders
-- This bypasses RLS but includes manual validation (same as RLS policy)
-- Use this when REST API anonymous requests don't work correctly
...
COMMENT ON FUNCTION create_guest_order IS 
  'Creates a guest order. Only works when auth.uid() IS NULL. Validates all requirements manually (same as RLS policy).';
