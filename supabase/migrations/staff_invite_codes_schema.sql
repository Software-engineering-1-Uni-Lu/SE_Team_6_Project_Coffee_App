...
-- Fast lookup for unused, non-expired invites
...
-- Future: Add RLS policy once role is accessible in policies
CREATE POLICY "Only admins and managers can create invites"
  ON staff_invite_codes
  FOR INSERT
  WITH CHECK (
    -- Allow insert if the user's role is admin or manager
    -- Note: This requires role to be stored in a way accessible to RLS
    -- For now, this is a placeholder - enforce in application code
    TRUE
  );
...
CREATE POLICY "Only admins can delete invites"
  ...
  FOR DELETE
  USING (
    -- For now, allow deletion - enforce in application code
...
-- FUTURE ENHANCEMENTS:
-- - Admin UI for creating/managing invite codes
-- - Email integration to send invite links
-- - Usage tracking and analytics
-- - Bulk invite creation
-- - Invite code expiration reminders
